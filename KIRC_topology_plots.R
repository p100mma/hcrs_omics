...
mse_cc_deg<- readRDS('KIRC_mse_deg_cc.rds')
...
names(test_stats)<- c('WGCNA', 'SVD93','SVD10','HR-5-4-15','PI-5-4-15','PII-5-4-15',
                       'SVD93+N','SVD10+N', 'HR-5-4-15+N','PI-5-4-15+N','PII-5-4-15+N')
test_titles<- c('WGCNA', 'SVD93','SVD10','HCR-5-4-15','HCS(n)-5-4-15','HCS(f)-5-4-15',
                       'SVD93+N','SVD10+N', 'HCR-5-4-15+N','HCS(n)-5-4-15+N','HCS(f)-5-4-15+N')
...
for (i in 1:length(test_stats)){
    jpeg(paste0('KIRC_cc_',names(test_stats)[[i]],'.jpg'), width= w, height= 360 )
...
CEX=1.6
for (i in 1:length(test_stats)){
    jpeg(paste0('KIRC_deg_',names(test_stats)[[i]],'.jpg'), width= w, height= 360 )
    par(mar = c(4, 5, 2, 1) + 0.1)
