...
PC= svd_X$u[,1:93] %*% diag(svd_X$d[1:93])
X_rec<- PC %*% t(svd_X$v[,1:93])
apply(X,2,var)-> V_X
...
saveRDS( list(PC=PC,
	      X_rec=X_rec,
	      X_rec_noise=X_rec_noise), 'KIRC_plainSVDreconstruct93.rds')
...
saveRDS( list(PC=PC,
	      X_rec=X_rec,
	      X_rec_noise=X_rec_noise), 'KIRC_plainSVDreconstruct10.rds')
