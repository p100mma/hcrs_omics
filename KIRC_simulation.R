load('KIRC_afterDecomposition.RData')
...
for (j in 1:ncol(PCmat_orig)) {
	PC_metalogs[[j]] <- readRDS( sprintf('PCs/KIRC_metalog_%d.rds',j))
	PCmat_hidden_norm[,j] <- readRDS( sprintf('PCs/KIRC_PChidden_normal%d.rds',j))
				}
target_terms= rep(5, ncol(PCmat_orig))
target_terms[[6]]=10 #1st PC of cluster number 2
...
PCmat_meta_vars<- apply(PCmat_cloned_meta,2,var)
for (j in 1:ncol(PCmat_cloned_meta))
	PCmat_cloned_meta[,j]= (PCmat_cloned_meta[,j]/sqrt(PCmat_meta_vars[j]))*sqrt(var(PCmat_orig[,j]))
...
X_meta=X_meta,
X_meta_noise=X_meta_noise), 'KIRC_reconstruction_results.rds')
